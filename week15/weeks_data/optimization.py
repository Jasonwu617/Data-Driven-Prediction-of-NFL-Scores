...
csv_files = [i for i in os.listdir('.') if '_averages.csv' in i]
for file in csv_files:
    ...
    test_set = pd.read_csv(file[:-12]+'averages_input.csv')
    # Keep only numeric columns
    ...
    df_teams.to_csv(file[:-12]+"predictions.csv", index=False)